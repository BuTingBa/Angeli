...
class angeli
{
		    ...
		    if($this->mysqli->affected_rows>0){
		        while ($row = $result->fetch_assoc())
		        {
		            $d = array(
		                ...
		                'DateCreated' =>date("Y-d-m H:i:s",$row['DateCreated']),
	...
	//获取支付订单列表
	public function getPayList($type="all",$count=20,$page=1,$sqltype='ok'){
	    $pageNum=($page-1)*$count;
	    //return $sql;
		if($sqltype=='del'){
			$time=strtotime("-10 day");
			$sql = "DELETE FROM angeli_pay WHERE orderTime<$time AND payStatus='待支付';";
			$result=$this->mysqli->query($sql) or die( $this->mysqli->error);
		}
		
	    switch ($type){
	        case 'all':
	            $sql = "SELECT * FROM angeli_pay ORDER BY orderTime  DESC  LIMIT $pageNum, $count";
	            break;
	        case 'wei':
	            $sql = "SELECT * FROM angeli_pay WHERE payStatus='待支付' ORDER BY orderTime  DESC  LIMIT $pageNum, $count";
	            break;
	        case 'ok':
	            $sql = "SELECT * FROM angeli_pay WHERE payStatus='已支付' ORDER BY orderTime  DESC  LIMIT $pageNum, $count";
	            break;
	        default:
	            $sql = "SELECT * FROM angeli_pay ORDER BY orderTime  DESC  LIMIT $pageNum, $count";
	            break;
	        ...
	        if($this->mysqli->affected_rows>0){
	            ...
	            while($row = $result->fetch_assoc()) {
	                $da = array(
	                    ...
	                    'orderTime' =>$this->uc_time_ago($row["orderTime"]),
	                    'wxOrderId' =>$row["wxOrderId"],
	                    'name' =>$row["name"]=='1'?'VIP':'安个利币',
	                    'number' =>$row["number"],
	                    'payFee' =>$row["payFee"]/100,
						'payStatus'=>$row['payStatus'],
						'leiji'=>$this->getLeijichongzhi($row["auid"])
	                );
	                $data[]=$da;
	            }
	            return $data;
	...
	public function getLeijichongzhi($auid)
	{
		$sql = "SELECT SUM(payFee) as count FROM angeli_pay WHERE payStatus ='已支付' AND auid=$auid group by auid";
		$result=$this->mysqli->query($sql) or die($this->mysqli->error);
		if(!$result){
		    //表示操作失败
		    $outmsg = array('code' =>'0','msg'=>'系统错误，Error：'.$this->mysqli->error,'data'=>'');
	...
	//获取提现申请列表
	public function getTixian($type="all",$count=20,$page=1){
	    $pageNum=($page-1)*$count;
	    //return $sql;
	    switch ($type){
	        case 'all':
	            $sql = "SELECT * FROM angeli_tixian ORDER BY addtime  DESC  LIMIT $pageNum, $count";
	            break;
	        case 'wei':
	            $sql = "SELECT * FROM angeli_tixian WHERE status='等待审核' ORDER BY addtime  DESC  LIMIT $pageNum, $count";
	            break;
	        case 'ok':
	            $sql = "SELECT * FROM angeli_tixian WHERE status='已提现' ORDER BY addtime  DESC  LIMIT $pageNum, $count";
	            break;
	        default:
	            $sql = "SELECT * FROM angeli_tixian ORDER BY addtime  DESC  LIMIT $pageNum, $count";
	            break;
	        ...
	        if($this->mysqli->affected_rows>0){
	            ...
	            while($row = $result->fetch_assoc()) {
	                $da = array(
	                    ...
	                    'addtime' =>$this->uc_time_ago($row["addtime"]),
	                    'status' =>$row["status"],
	                    'fee' =>$row["fee"]?$row["fee"]:'未操作',
	                    'operator' =>$row["operator"]?$row["operator"]:'未操作',
	                    'openid' =>$row["openid"],
						'tixianto' =>$row["openid"]=='微信'?'微信':'支付宝'
	                );
	                $data[]=$da;
	            }
	            return $data;
	...
	/**
	 * 获取管理员信息
	 * @param $auid 欲查询的微信openID
	 * @return array|bool
	 */
	public function getAdminInfo($openid)
	{
	    $sql = "select * from angeli_admin WHERE wxopenid='$openid'";
	    $result=$this->mysqli->query($sql) or die($this->mysqli->error);
	    if(!$result){
	        //表示操作失败
	        return false;
	 ...
	 * @param {Object} $auid
	 */
	public function addadmin($openid,$name,$phone=123,$note='无')
	{
		$sql="INSERT INTO angeli_admin (adminname,wxopenid,phone,note) VALUES ('$name','$openid','$phone','$note')";
		$result=$this->mysqli->query($sql) or die($this->mysqli->error);
		if(!$result){
		    //表示操作失败
		    return false;
	 ...
	 * @return bool
	 */
	public function setPostInfo($postid,$key,$val)
	{
	    $sql = "UPDATE angeli_posts SET $key='$val' WHERE PostsId=$postid";
		//echo $sql;
	    $result=$this->mysqli->query($sql) or die($this->mysqli->error);
	    if(!$result){
	        //表示操作失败
	 ...
	 * @return bool
	 */
	public function setUserInfo($auid,$key,$val)
	{
	    $sql = "UPDATE angeli_user SET $key='$val' WHERE AuId=$auid";
	    $result=$this->mysqli->query($sql) or die($this->mysqli->error);
	    if(!$result){
	        //表示操作失败
	        return false;
    ...
    *   参数：查询关键字类型，查询关键字
    */
    public function getUserInfo($type='auid',$keyword,$auid='0')
    {
        switch ($type) {
            case 'wxid':
   ...
   */
    ...
    *
    */
    public function getUserList($page='1',$count='50',$pxtype=1)
    {
		$paixu=$_SESSION['paixu'];
		$pageNum=($page-1)*$count;
		switch($pxtype){
			case 1:
				$sql="SELECT *  from angeli_user ORDER BY DateCreated  DESC LIMIT $pageNum, $count";
				break;
			case 2:
				$sql="SELECT * from angeli_user ORDER BY DateCreated  ASC  LIMIT $pageNum, $count";
				break;
			case 3:
				$sql="SELECT * from angeli_user ORDER BY Points  DESC  LIMIT $pageNum, $count";
				break;
			case 4:
				$sql="SELECT * from angeli_user ORDER BY Points  ASC  LIMIT $pageNum, $count";
				break;	
			case 5:
				$sql="SELECT * from angeli_user ORDER BY ZhongcaoCount  DESC  LIMIT $pageNum, $count";
				break;
			case 6:
				$sql="SELECT * from angeli_user ORDER BY ZhongcaoCount  ASC  LIMIT $pageNum, $count";
				break;
			case 7:
				$sql="SELECT * from angeli_user ORDER BY VIPEndTime  DESC  LIMIT $pageNum, $count";
				break;
			case 8:
				$sql="SELECT * from angeli_user ORDER BY VIPEndTime  ASC  LIMIT $pageNum, $count";
				break;
			case 9:
				$sql="SELECT * from angeli_user ORDER BY AuId  DESC  LIMIT $pageNum, $count";
				break;
			case 10:
				$sql="SELECT * from angeli_user ORDER BY AuId  ASC  LIMIT $pageNum, $count";
				break;
			default:
				$sql="SELECT *  from angeli_user ORDER BY DateCreated  DESC LIMIT $pageNum, $count";
				break;
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {
                $ddata = array(
                    ...
                    'Gender' =>$row['Gender']=='1'?'男':'女',
                    'UserType' =>$this->isVip($row["VIPEndTime"]),
                    'VIPEndTime' =>date("Y-m-d H:i:s",$row["VIPEndTime"]),
                    'Status' =>$row['Status']=='1'?'正常':'封禁',
                    'BanDeadline' =>$row['BanDeadline'],
